...
class TextLoader {
	...
	public static function LoadBuildCode(string $text) : BuildCode {
		...
		for($i = 0; $i < 3; $i++) {
			if(!TextLoader::EatToken($view, '_')) {
				...
				for($j = 0; $j < 3; $j++)
					$choices[$j] = TraitLineChoice::from((($mixed >> (4 - $j * 2)) & 0b00000011));
	...
	{
		$set = new WeaponSet();
		if(!TextLoader::EatToken($text, '_')) $set->MainHand = WeaponType::from(WeaponType::_FIRST() + TextLoader::DecodeAndAdvance($text));
	...
	private static function LoadProfessionSpecific(StringView $text, Profession $profession) : IProfessionSpecific
	{
		switch($profession)
		{
	...
	public static function WriteBuildCode(BuildCode $code) : string
	{
		...
		{
			TextLoader::EncodeWeaponSetAndAdvance($destination, $code->WeaponSet1);

			if(!$code->WeaponSet2->HasAny()) $destination .= '~';
			else
			{
				TextLoader::EncodeWeaponSetAndAdvance($destination, $code->WeaponSet2);
			}
		}
	...
	private static function EncodeWeaponSetAndAdvance(string &$destination, WeaponSet $set) : void
	{
		if($set->MainHand === WeaponType::_UNDEFINED) $destination .= '_';
		else {
			$destination .= TextLoader::CHARSET[$set->MainHand->value - WeaponType::_FIRST()];
	...
	private static function EncodeInfusionsAndAdvance(string &$destination, BuildCode $weaponRef) : void
	{
		...
		for($i = 0; $i < ALL_INFUSION_COUNT; $i++)
		{
			...
			if($weaponRef->Infusions[$i] !== $lastInfusion)
			{
				...
				{
					TextLoader::EncodeOrUnderscoreOnZeroAndAdvance($destination, $lastInfusion, 3);
					$destination .= TextLoader::CHARSET[$repeatCount];
				}
		...
		TextLoader::EncodeOrUnderscoreOnZeroAndAdvance($destination, $lastInfusion ?? StatId::_UNDEFINED, 2);
		if($repeatCount > 1)
			$destination .= TextLoader::CHARSET[$repeatCount];
	}
