...
class PerProfessionData {
	...
	public static function ByProfession(Profession $profession) : PerProfessionData
	{
		switch ($profession) {
	...
	function ReloadFromOfflineFiles(Profession $profession) : void
	{
		$file = fopen("offline/pallettes/{$profession->name}.csv", 'r', use_include_path: true);
	...
	public static function Reload(Profession $profession, bool $skipOnline = false) : void
	{
		...
		if(!$skipOnline)
		{
			try
			{
				$professionData = APICache::Get("/professions/{$profession->name}", '2019-12-19T00:00:00.000Z');
