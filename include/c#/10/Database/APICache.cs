...
public static partial class APICache {
	...
	public static StatId ResolveStatId(int itemId)
	{
		var itemData = _client.WebApi.V2.Items.GetAsync(itemId).Result;
	...
	public static TraitLineChoice ResolvePosition(int? traitId)
	{
		if(!traitId.HasValue) return TraitLineChoice.NONE;

		var traitData = _client.WebApi.V2.Traits.GetAsync(traitId.Value).Result;
