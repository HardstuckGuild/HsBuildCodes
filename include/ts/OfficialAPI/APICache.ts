...
class APICache {
	...
	public static Get(path : string, schemaVersion : string = 'latest') : any
	{
	...
	public static ResolveWeaponType(itemId : ItemId) : WeaponType
	{
		const itemData = APICache.Get("/items/"+itemId);
		console.assert(itemData.type === "Weapon", "Item is not a weapon:", itemData);
	 ...
	 */
	public static ResolveStatId(itemId : ItemId) : StatId
	{
		const itemData = APICache.Get("/items/"+itemId);
		return itemData.details.infix_upgrade?.id ?? StatId._UNDEFINED;
	}

	public static ResolvePosition(traitId : TraitId) : TraitLineChoice
	{
		if(!traitId) return TraitLineChoice.NONE;

		const traitData = APICache.Get("/traits/"+traitId);
		return (traitData.order + 1) as TraitLineChoice;
	}

	public static ResolveWeaponSkill(code : BuildCode, effectiveWeapons : WeaponSet, skillIndex : number) : SkillId
	{
		let weapon : any = null;
		if(skillIndex < 3)
		{
			...
			const professionData = APICache.Get("/professions/"+Profession[code.Profession]);
		...
		else
		{
			...
			//NOTE(Rennorb): this isnt outside of the if to allow early bail if the guard condition isnt met.
			const professionData = APICache.Get("/professions/{code.Profession.name}");
			if(effectiveWeapons.OffHand !== WeaponType._UNDEFINED)
	...
	public static ResolveTrait(spec : Specialization, traitSlot : TraitSlot) : TraitId
	{
		...
		const allSpecializationData = APICache.Get('/specializations?ids=all');
